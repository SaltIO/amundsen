...
@metadata_blueprint.route('/update_provider_tags', methods=['PUT', 'DELETE'])
def update_provider_tags() -> Response:

    @action_logging
    def _log_update_provider_tags(*, uri_key: str, method: str, tag: str) -> None:
        pass  # pragma: no cover
    ...
    try:
        ...
        uri_key = get_query_param(args, 'key')
        tag = get_query_param(args, 'tag')
        url = f'{data_provider_endpoint}/{uri_key}/tag/{tag}'

        _log_update_provider_tags(uri_key=uri_key, method=method, tag=tag)
        ...
        if status_code == HTTPStatus.OK:
            ...
            message = f'Encountered error: {method} provider tag failed'
            logging.error(message)
