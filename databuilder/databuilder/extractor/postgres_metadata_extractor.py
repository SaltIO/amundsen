    ...
    def get_sql_statement(self, use_catalog_as_cluster_name: bool, where_clause_suffix: str) -> str:
        ...
        return """
            WITH Objects AS (
                ...
                WHERE c.relkind IN ('r', 'v', 'm', 'p', 'f') AND i.inhrelid IS NULL {where_clause_suffix} -- Exclude child partitions
            ),
            Columns AS (
                SELECT
                ...
                WHERE c.relkind IN ('r', 'v', 'm', 'p', 'f') AND a.attnum > 0 {where_clause_suffix}
            ),
            Final AS (
                SELECT
                    o.cluster,
